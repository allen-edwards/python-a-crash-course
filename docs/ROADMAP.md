...
- **YouTube-hosted video scripts (`/api/generate-script`, `genScript()`) — archived, not removed (2026-09-13).** Original purpose: generate a YouTube-style video script per chapter for Allen to record and host on a YouTube channel — superseded by F14's in-app Video mode, and Allen has decided against a YouTube channel for this app (external dependency risk: policy changes, copyright strikes outside his control). The "Generate video script" button and the stale "Video lesson coming soon!" placeholder were removed from all 11 chapters' lesson pages and replaced with a pointer to the real F14 Video mode. The backend route (`app.py`) and its now-unreferenced frontend code (`genScript()`, `copyScript()`, `#scriptModal`) were deliberately left intact rather than deleted, per the same "confirm genuinely dead before removing" discipline as the Skulpt cleanup - **decision point: if still unused by v8.0, remove it entirely; if a real future use emerges first, this note can simply be deleted.**
