...
## Known bugs
- **"Mark complete" does not verify the challenge or quiz were actually attempted (found 2026-08-31).** Allen was able to mark chapters complete and receive full XP without writing any code or answering any quiz questions. This defeats the app's own progression system and undermines the content-integrity work done on the challenges/quizzes — a student can currently skip all of it and still unlock every chapter. Not yet fixed; root cause not yet investigated (likely in the completion/XP-award logic in `app.py` or `index.html`'s progress handling — needs Claude Code to locate the exact code path before proposing a fix).
