...
| F14b | Video mode narration audio, produced by Allen via Speechify from the same per-chapter script used for F14. When wired in, playback should sync to the audio rather than an internal timer. | Nice | **Audio produced for all 11 chapters (2026-09-13), not yet wired into the app.** Plain Text-to-Speech (verbatim, single voice) via Speechify - not the "Podcast"/"Lecture" feature (that one rewrites the text; tried first, kept separately as a future companion-podcast idea, see ROADMAP.md Ideas/someday). Files live in `audio/ch1.mp3`–`ch11.mp3`. Still needed: a Flask static route to serve them (matching the existing `/vendor/<filename>` pattern) and wiring `CHAPTER_SCRIPTS`/the Video-mode player to load and sync against `audio.currentTime` instead of the internal timer. |
