...
| F14 | A single toggleable lesson panel, accessible from anywhere in the app (including the sandbox) — replaces the current text-based pop-out. Two modes: **Video** (code-typing animation + captions, scrubbable progress bar, no audio in v1) and **Text** (readable lesson content, as today). Both modes render from one shared script per chapter. | Nice | **v1 Done** (2026-08-31, Chapter 1). Beat format `CHAPTER_SCRIPTS` (narration / code / run / output, each with `typeSpeedMs` / `holdMs`) + `getBeatDuration()`. Panel replaces the old text pop-out, reachable from every tab incl. Sandbox. Text mode renders the full script; Video mode plays a silent code-typing animation in an editor+terminal split, with a play/pause button and a click/drag-scrub progress bar, explicit final-state hold, and replay-from-start. Mode toggle still resets to top. **F14a** (position-aware toggling) and **F14b** (Speechify narration audio) remain separately deferred. Chapters 2–11 need their own `CHAPTER_SCRIPTS` entries. |
