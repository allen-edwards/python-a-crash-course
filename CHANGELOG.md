...
### Fixed
- **Quiz XP could be re-earned by retrying the quiz.** `QS.xpDone` reset every time `renderQuiz()` ran (the Retry button) and was never checked against `S.quizScores`, so a student could pass the same quiz repeatedly for unlimited XP - the same kind of "XP without the real work" hole as the 2026-09-13 "Mark complete" bug, through a different path, and one that bypasses any threshold values. Quiz XP is now awarded once per chapter: `S.quizScores[chapter] >= 3` is the persistent "already earned" record. A repeat pass shows "Quiz XP already earned for this chapter" instead of "+N XP", and the quiz header says so too. Verified: fail = 0 XP, first pass = +11 (Ch 1), retry pass = 0, repeated retry = 0.
- **A saved current chapter that was locked still opened on startup.** Startup rendered `CHAPTERS[S.cur]` with no lock check, so a student whose saved chapter re-locked (thresholds moved, or a hand-edited save) saw its lesson and quiz. On load, a locked `S.cur` now falls back to the highest unlocked chapter and the change is saved.
- **Ch 10 and Ch 11 unlock thresholds were unreachable on a clean run.** A single clean pass (quiz XP + completion XP per chapter, `1.7 × reward`) earns at most 351 XP before Ch 10 under the OLD thresholds and 410 under the NEW ones, while the old threshold was 360 and the `+2 × reward` rule would now give 420 (Ch 11: 490 needed of 470 available). The rule outgrows what a chapter can pay. Ch 10 and Ch 11 are therefore set to **400 and 460** (about 10 XP of slack each), deliberately overriding the rule. Ch 9 remains tight (2 XP of slack).

### Added
- **One-time guarded XP backfill (`xpModel: 2`).** Saves made before this change have no `xpModel` marker. On first load, `migrateXpModel()` adds the reward difference (new reward minus the model-1 reward) for every completed chapter and every quiz already passed (quiz XP = `round(reward × 0.7)`), capped at 500, then sets `xpModel: 2` so it never runs twice. This keeps existing students from being pushed behind the moved thresholds by the rebalance itself. Example, Allen's real save: 336 XP → 387 XP, Chapter 9 (now 340) stays open. `app.py` deliberately does not put `xpModel` in its defaults (that would make old saves look already migrated). A fresh install or Reset starts at model 2.
- Verified in a live browser against a scratch copy: Allen's real save migrates once and stays 387 on reload; a locked saved chapter falls back to Ch 8; a full clean run through all 11 chapters opens every chapter in order and ends at 500 XP; cap handling (495 + 25 → 500); a brand-new install has no error and marker 2.
